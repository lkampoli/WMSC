...
class Model(object):
    ...
    def train(self):
        ...
        if self.restart != 1:
            ...
            # cost_fun1 = "np.mean((np.einsum('...ij,...ij', EXPR00 * V01 + EXPR01 * V02 + EXPR02 * V03 + EXPR03 * V04 + EXPR04 * V05 + EXPR05 * V06 + EXPR06 * V07 + EXPR07 * V08 + EXPR08 * V09 + EXPR09 * V10, V01) - np.squeeze(R))**2)"
            cost_fun1 = "np.sum(((np.einsum('...ij,...ij', EXPR00 * V01 + EXPR01 * V02 + EXPR02 * V03 + EXPR03 * V04 + EXPR04 * V05 + EXPR05 * V06 + EXPR06 * V07 + EXPR07 * V08 + EXPR08 * V09 + EXPR09 * V10, V01) - np.squeeze(R)) / np.sqrt(DATALEN))**2)"
            ...
            if self.restart != 2:
                ...
                # Symbols to load
                #frozens = [I01sym, I02sym, I03sym, I04sym, I05sym, V01sym, V02sym, V03sym, V04sym, V05sym, V06sym, V07sym, V08sym, V09sym, V10sym, Rsym]
                frozens = [I01sym, I02sym, I03sym, I04sym, I05sym,
                           ...
                           #Q02sym, Q03sym, Q05sym, Q06sym, Q10sym,
                           Rsym]
                actives = []
